...
from actions.actions import actionDTOMapConfig, GAME_ACTIONS, PLAYER_ACTIONS
...
class KafkaGameManager(GameManager):
    ...
    async def process_event(self, user_id, data: Any):
        ...
        try:
            action_dto: BaseActionDto = action_dto_class(action=action, params=params)
    ...
    async def __process_player_death(self, player: Player):
        ...
        self._game.users_player_controllers.pop(player.user_id, None)
        await player.notify_observers()
        await self.__update_payer_data(player)

    async def __get_full_map(self, params: tuple[str] | None = None):
        map_id = params[0] if params else None
        location = self._game.get_location(map_id)
        full_map = location.get_map()
        map_id = full_map.map_id
        ...
        await self._output_queue.put({
            ...
            "key": map_id,
            "value": {"location_id": map_id,
                      "location_size": location.get_location_size(),
                      "location_data": full_map_data}
        })

    async def __get_player(self, user_id, params: tuple[str]) -> Player | None:
        char_name = params[0]
        player_controller = await self._game.get_player_controller_by_user_id(user_id)
        if player_controller:
            player = player_controller.get_player()
            player.is_active = True
            return player

        player_controller = await self._game.get_player_controller_by_char_name(char_name)
        if player_controller:
            if player_controller.get_player().user_id == user_id:
                player = player_controller.get_player()
                player.is_active = True
                await self._game.assign_character_to_user(user_id, char_name)
                return player
            return None
        ...
        player = character_model_to_player(char, self.context.interaction_system, self.context)
        player.is_active = True
        return await self._game.return_character_to_game(player, char.stats.location_id)

    async def __create_player(self, params: tuple[str], user_id):
        name = params[0]
    ...
    async def __apply_event_to_player(self, user_id, action_dto: BaseActionDto):
        ...
        if inspect.iscoroutinefunction(method):
            if action_dto.params is not None:
                result = await method(action_dto.params)
            else:
                result = await method()
        else:
            if action_dto.params:
                result = method(action_dto.params)
            else:
                result = method()
    ...
    async def __apply_event_to_game(self, action_dto: BaseActionDto, user_id: str):
        ...
        match action_dto.action:
            ...
            case "create_player":
                player = await self.__create_player(action_dto.params, user_id=user_id)
                if player:
                    await player.notify_observers()
                else:
                    await self.__send_char_not_found_message(user_id)
            ...
            case "get_full_map":
                await self.__get_full_map(action_dto.params)

            case "get_player":
                player = await self.__get_player(user_id, action_dto.params)
                if player:
                    await player.notify_observers()
                else:
                    await self.__send_char_not_found_message(user_id)
